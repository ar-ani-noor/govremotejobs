...
## Notes
- Ohio, Georgia, and several other states run Workday for civil-service hiring; if state coverage becomes a priority before NY/CA/TX/WA improve, probing `{tenant}.wd*.myworkdayjobs.com` for state tenants is the highest-yield path since the adapter already exists.
